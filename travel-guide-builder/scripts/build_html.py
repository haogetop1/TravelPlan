...
def main():
    ...
    # ── 地图源：优先小红书实拍地图 assets/maps_xhs/，缺失才回落高德截图 ──
    XHS_MAPS = os.path.join(RB, 'assets', 'maps_xhs')
    xhs_have = set()
    if os.path.isdir(XHS_MAPS):
        xhs_have = {f[:-4] for f in os.listdir(XHS_MAPS) if f.endswith('.png')}
    ...
    def _pick(m):
        ...
        if slug in xhs_have:
            swapped.add(slug)
            return 'assets/maps_xhs/%s.png' % slug
        return m.group(0)

    out_html = re.sub(r'assets/maps/([A-Za-z0-9_]+)\.png', _pick, out_html)

    all_refs = set(re.findall(r'assets/maps_xhs/([A-Za-z0-9_]+)\.png', out_html)) | \
               set(re.findall(r'assets/maps/([A-Za-z0-9_]+)\.png', out_html))
    print('地图源: 小红书 %d 张 %s' % (len(swapped), sorted(swapped)))
    print('地图源: 高德回落 %d 张 %s' % (len(all_refs - swapped), sorted(all_refs - swapped)))
