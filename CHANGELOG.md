...
## [2026-09-18] · 跨宿主平台通用化：平台兼容层 + 多平台安装器 + 通用入口

### 背景

用户提问「这个技能其它 agent 平台能用吗」。先做了一次**移植性审计**
（全部 47 个文件 × 12 类平台依赖标记），结论是：

> **核心逻辑本来就没有平台绑定** —— xlsx / HTML / PPT 渲染、内容抽取、价格解析
> 全是纯 Python。所有平台假定都堆在**两处**：① 浏览器会话层
> （原生 Chrome 探测、进程管理、会话目录）② 脚本之间 import 的写死路径。

所以「不能用」不是因为算法，而是因为**安装约定、入口文件、以及几个写死的路径**。

### 🎯 范围（明确边界，不夸大）

**只做 Windows 10/11 + 任意宿主 agent 平台。** macOS / Linux **不在范围内** ——
代码里留了极少量 POSIX 兜底分支（避免在 WSL 里直接崩），但**未经真机验证**，
文档里也写明不要依赖。

### ✨ 新增 `xhs-humanized-collect/scripts/platform_compat.py`（唯一实现点）

把散落的平台假定收敛成一个模块，换平台只改这一处：

| 能力 | 解决什么 |
|---|---|
| `AGENT_TARGETS` 宿主注册表 | 7 个宿主（WorkBuddy / Claude Code / Codex / Cursor / Gemini / Windsurf / 通用）各自的技能目录、是否认 SKILL.md、要生成哪些入口文件（含 `project` / `global` 作用域） |
| `find_chrome()` | 原生 Chrome / Edge 探测（多路径候选 + `CHROME_PATH` 覆盖）。**原生内核是硬要求**：携程/神州的指纹风控会拦 Playwright 自带 |
| `session_dir()` | 会话目录，**不再绑 WorkBuddy 的 `.workbuddy`**：`SCRAPE_SESSION_DIR` >（旧的 `<cwd>/.workbuddy` 若已用过则沿用）> `%LOCALAPPDATA%\agent-scrape-session`。旧目录沿用这一条是刻意的 —— 升级不能让用户「突然又要重新扫码」 |
| `run_bytes()` / `decode_console()` | 中文 Windows 的 `tasklist`/`taskkill` 输出是 GBK，`text=True` 会在 reader 线程抛 `UnicodeDecodeError` 被吞 → 「进程活着却判成已死」。一律按字节读回再自行解码 |
| `pid_alive()` / `kill_tree()` / `detach_kwargs()` | 进程存活、结束进程树、脱离父进程 |
| `skills_roots()` / `sibling_script_dirs()` / `add_sibling_to_path()` | 技能互相 import 的目录发现（多候选，不再写死绝对路径） |
| `launch_instructions()` | 生成「请用户自己启动浏览器」的跨平台命令与说明（含 `.bat` 编码坑的警告） |
| `describe()` / `--selfcheck` / `--where` / `--agents` | 排障与自检（40 项不变量） |

设计上刻意让**关键函数带 `platform` 参数** —— 跨平台代码如果只能靠在三种机器上
各跑一遍来验证，等于没法验证；带参数就能在单机上把分支全测到。

### ⚙️ 浏览器层重构（去平台化，行为保持不变）

- `human_act.py`：删掉硬编码的 `CHROME_CANDIDATES` → `PC.find_chrome()`；
  `session_dir()` 委托兼容层；`launch_chrome()` 用 `PC.launch_argv` + `PC.detach_kwargs()`。
  自检新增 4 项「平台中立性」断言（用**行为断言**，不是源码字符串匹配 —— 见下）。
- `session_daemon.py`：`pid_alive` / `kill_tree` / `chromium_path` / `pick_exe` / 会话目录
  全部走兼容层。并且**刻意保留** tasklist / taskkill / schtasks 的 Windows 实现
  （Windows 是支持平台，这些是正确做法，只是收敛到一处）。
- `xhs_login.py`：登录态默认**不再写进技能目录**（技能目录是安装物，写脏后会跟着
  同步进公开仓库）→ 落到会话目录；旧的「技能旁 `xhs_profile`」若已存在则继续沿用。
- `xhs_collect_photos.py`：产物从「技能目录下」改为「当前工作目录下」（可用
  `XHS_PHOTOS_OUT` 覆盖）；profile 定位改成候选列表（含历史约定，不丢老登录态）。
- `ctrip_hotel.py` / `zuche_h5_quote.py`：import 自举改为「按并列安装布局推 + 多平台候选」，
  报错时把**尝试过的目录全列出来**，而不是只说一句「找不到」。

### 📦 新增 `install.py` + `AGENTS.md` + `PLATFORMS.md`

- **`install.py`**：`--target auto|all|<平台>`、`--project`、`--dry-run`、`--force`、`--uninstall`。
  认 SKILL.md 的宿主直接铺技能；不认的宿主**生成浓缩操作说明**（含硬约束与常见坑）。
  两条安全设计：**不覆盖已存在的技能目录**（除非 `--force`）、
  **不覆盖非本工具生成的入口文件**（生成的入口带标记，`--uninstall` 只删带标记的）。
- **`AGENTS.md`**（仓库根）：跨工具事实标准入口，含完整硬约束与「跨平台通用的坑」表。
- **`PLATFORMS.md`**：支持矩阵 + 已知限制 + 自检命令。

### 📄 文档去平台化

`WorkBuddy` 托管 Python 路径、`~/.workbuddy/skills/...`、`present_files` 等平台专有表述
全部改成中性写法（「宿主有展示文件能力就用，例如部分平台的 `present_files`；没有就给路径」）。
两份 `SKILL.md` 顶部新增「宿主平台」一节；「文件约定」改为「代码在技能目录 / 产物在会话目录」。

### 🐛 过程中被自检抓到的三个真 bug（都已修）

1. **自检项自我指涉**：写了 `"CHROME_CANDIDATES" not in src`，而**这行字符串本身就在
   src 里** → 假 FAIL。改为行为断言（临时设 `SCRAPE_SESSION_DIR`，验证解析结果）。
2. **入口路径多出一个字面量 `~` 目录**：注册表里 global 项写成 `~/.codex/AGENTS.md`
   （为了可读性），而 `entry_files_for` 已经把 `~` 展开成 home，结果拼出
   `<home>\~\.codex\AGENTS.md`。已剥前导 `~` 并加不变量断言。
3. **安装目标与发现路径不一致**：`generic` 装到 `agent-skills/` 但发现只查
   `agent-skills/skills/` → 会「装上了却找不到」。已统一为 `default_skills_root()` 单一来源。

### ✔️ 验证方式（不是只看 diff）

- 三项自检**在仓库副本上**跑（验证打包物而非只验本地）：
  `platform_compat --selfcheck` 40 项 PASS、`human_act --selfcheck` PASS、
  `xhs_collect_photos --selfcheck` PASS。
- **端到端回归**：真拉起原生 Chrome → attach → 读回 DOM → 退出 `with` 块后
  浏览器仍在（确认「只关标签页、绝不关浏览器」在重构后仍成立）。
- **安装器沙箱实测**：把 `USERPROFILE`/`LOCALAPPDATA` 指向临时目录，跑
  `--target all` 装 7 个宿主 → 检查生成的技能目录与入口文件 →
  `--uninstall`。两个安全性质都验证到：用户自有的 `AGENTS.md` 全程未被覆盖，
  卸载后只剩它一个文件。
- 消费者脚本（`ctrip_hotel.py` / `zuche_h5_quote.py`）分别从**技能目录**和
  **项目目录**两种布局各跑一次 `--help`，确认 import 自举都成立。

### 🔒 安全

推送前对仓库全部文本文件扫描：GitHub token / 32 位 hex key / 私钥 / 邮箱 /
本机绝对路径 / 本机用户名 → **0 命中**。
`.gitignore` 补上会话与产物目录（`browser_profile/` `cookies_*.json`
`session_daemon.json` `calibration.json` `roadbook/` `_raw/` `*.log`）。

### 📌 一件事需要你决定

仓库目前**没有 `LICENSE` 文件**，技能 frontmatter 也没有 `license` 字段。
公开仓库不带许可证，别人无法从法务上判断能否使用 —— 建议补一个（例如 MIT），
但这属于你的决定，我没有替你选，只在 `PLATFORMS.md` 里记了这条待办。
...
### 📐 规则固化：冻结三条 + 富文本默认开启（用户 2026-09-17 明确）

**冻结规则**（与上一条回退结果一致，现已写进源码注释与 SKILL.md）：

| sheet | 冻结 | 源码 |
|---|---|---|
| 全部行程 | 首行 + A、B 列（便于横向滑动） | `C2` |
| 费用明细 | 只冻结首行 | `A2` |
| 人均预算 / 物品清单 / 景点地图 | 不冻结 | 无 |

**富文本改为默认开启**：`put()` 的 `rich` 参数默认从 `False` 改成 **`True`**，
于是 `richify()` 对**所有 sheet** 生效（此前只有 `全部行程` 的非首列显式传了 `rich=True`）。
需要纯文本时显式 `put(..., rich=False)`。

**实测**（构造含 `【】`/`〔〕` 的五表 fixture 跑完整构建）：

- 冻结：`全部行程 C2` / `费用明细 A2` / 其余 `None` ✅
- 富文本分布：全部行程 3 格、人均预算 2 格、费用明细 1 格、物品清单 2 格、景点地图 2 格；
  `【…】` = 红色加粗 `00C00000`、`〔…〕` = 棕色加粗 `007A5C3E` ✅
- 回读注意：必须 `load_workbook(path, rich_text=True)`，否则富文本被还原成 `str`
